(() => {
  ...
  async function submitFeedback(event){
    ...
    try{
      ...
      const result=await send({name:data.get('name'),email:data.get('email'),hub:data.get('hub'),category:data.get('category'),message:data.get('message'),documentUrl:data.get('documentUrl'),attachment});
  ...
  function renderInbox(items){
    ...
    inbox.innerHTML=items.map(item=>`<article class="feedback-item" data-feedback-id="${escapeHtml(item.id)}"><header><div><b>${escapeHtml(item.reference)}</b><span>${escapeHtml(new Date(item.submittedAt).toLocaleString())}</span></div><select aria-label="Feedback status"><option value="new" ${item.status==='new'?'selected':''}>New</option><option value="reviewed" ${item.status==='reviewed'?'selected':''}>Reviewed</option><option value="follow-up" ${item.status==='follow-up'?'selected':''}>Follow-up</option><option value="closed" ${item.status==='closed'?'selected':''}>Closed</option></select></header><div class="feedback-tags"><span>${escapeHtml(item.hub)}</span><span>${escapeHtml(readableCategory(item.category))}</span><span>${escapeHtml(item.name)}</span></div>${item.email?`<a class="feedback-email" href="mailto:${escapeHtml(item.email)}">${escapeHtml(item.email)}</a>`:'<span class="feedback-email missing">Email not collected in this earlier submission</span>'}${item.message?`<p>${escapeHtml(item.message)}</p>`:''}<footer>${item.documentUrl?`<a href="${escapeHtml(item.documentUrl)}" target="_blank" rel="noopener noreferrer">Open document link ↗</a>`:''}${item.attachment?`<a href="${escapeHtml(item.attachment.url)}" download>Download ${escapeHtml(item.attachment.name)} · ${formatSize(item.attachment.size)}</a>`:''}<small>Submitted by ${escapeHtml(item.submittedBy)} · ${escapeHtml(readableStatus(item.status))}</small></footer></article>`).join('');
  }
