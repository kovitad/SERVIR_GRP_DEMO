...
function runAssessment(){
  ...
  setTimeout(()=>{
    assessmentComplete=true;running=false;refreshScenarioUI();mapCanvas.classList.add('results-visible');$('#layerCount').textContent='6';
    const d=scenarioData[selectedScenario];const a=currentAOI();
...
function placeCandidate(){
  if(!candidateMode)return;candidateMode=false;mapCanvas.classList.remove('candidate-mode');mapCanvas.classList.add('candidate-visible');$('#layerCount').textContent='7';
...
function showRiskMap(){
  ...
  if(mapCanvas.classList.contains('risk-view'))return showPlanningView();mapCanvas.classList.add('risk-view');$('#legendTitle').textContent='Risk-map legend';$('#downloadRiskMap').textContent='Planning view';userMessage('Show the risk map as further information.');aiMessage('<p>The secondary risk-map view shows high/red, medium/yellow and low/green points. It supports the candidate-place and vulnerable-people review but does not replace either viewpoint. Low/green does not mean safe.</p>');openDetail('centres');
}
...
function send(){
  ...
  setTimeout(()=>{
    ...
    else if(q.includes('people')||q.includes('vulnerab')||q.includes('age')){if(assessmentComplete){switchTab('people');openDetail('people');}else aiMessage('<p>Select the AOI and run the assessment first.</p>');}
